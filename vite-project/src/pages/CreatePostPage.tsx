...
const CreatePostPage: React.FC = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    user_id: Yup.number().required('ini hrsnya auto generate berdasarkan id session(?), tpi sistem login kan blm ada ya'),
  });

  const onSubmit = (data: typeof initialValues) => {
    const postData = { ...data, user_id: 1 }; // ID user sementara karna blm ada login sistem
    axios.post("http://localhost:3001/posts", postData).then(() => {
      alert("Post Created");
      navigate("/");
    }).catch(err => {
      console.error(err);
